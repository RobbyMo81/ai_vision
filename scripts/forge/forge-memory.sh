...
CREATE TABLE IF NOT EXISTS discoveries (
  ...
  CHECK(type IN ('PATTERN','GOTCHA','BLOCKER','DECISION','DEPENDENCY','CONVENTION'))
);
...
memory_print_startup_report() {
  ...
  total_sessions=$(sqlite3 "$FORGE_DB" "SELECT COUNT(*) FROM forge_sessions;" 2>/dev/null || echo "0")
  prior_discoveries=$(sqlite3 "$FORGE_DB" "SELECT COUNT(*) FROM discoveries;" 2>/dev/null || echo "0")
  unread_messages=$(sqlite3 "$FORGE_DB" "SELECT COUNT(*) FROM agent_messages WHERE read_at IS NULL;" 2>/dev/null || echo "0")

  local last_story_completed
  last_story_completed=$(sqlite3 "$FORGE_DB" \
  ...
  local ctx_count
  ctx_count=$(sqlite3 "$FORGE_DB" "SELECT COUNT(*) FROM context_store;" 2>/dev/null || echo "0")
...
memory_health_check() {
  local version
  version=$(sqlite3 "$FORGE_DB" "SELECT value FROM db_meta WHERE key='schema_version';" 2>/dev/null || echo "MISSING")
